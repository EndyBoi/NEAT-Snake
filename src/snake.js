class Snake {
	...
	move(game) {
		...
		if (output[0]) {
			this.brain.score += isFoodLeft
				? this.scoreModifiers.movedTowardsFood
				: this.scoreModifiers.movedAgainstFood
			switch (this.direction) {
				case 'up':
					this.direction = 'left'
					break
			...
			this.brain.score += isFoodForward
				? this.scoreModifiers.movedTowardsFood
				: this.scoreModifiers.movedAgainstFood
		}
