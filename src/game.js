class Game {
	...
	updateGameStatus() {
		...
		const snakeHitWall =
			snakeHead[0] < 1 ||
			snakeHead[0] > this.unitsPerRow ||
			snakeHead[1] < 1 ||
			snakeHead[1] > this.unitsPerRow
		const snakeHitTail = this.snake.segments.some(
			(s, i) =>
				s[0] === snakeHead[0] && s[1] === snakeHead[1] && i !== snakeHeadIndex
		)
